...
public class ExtentReportsListener implements ITestListener {
    ...
    public void onTestSuccess(ITestResult result) {
        ...
        ExtentReportUtils.extentTestPass("<span style='color:green; font-weight:bold'>Test başarıyla tamamlandı. (Test completed successfully.) " +
                "</span>" + passIsareti);
    ...
    public void onTestFailure(ITestResult result) {
        ...
        ExtentReportUtils.extentTestFail("<span style='color:red; font-weight:bold'>Test başarısız oldu! (The test failed.)</span>" + failIsareti);
        ExtentReportUtils.extentTestFail(result.getName());
        // Raporlama
        try {
            ExtentReportUtils.addScreenShotToReport();
        } catch (Exception e) {
            ExtentReportUtils.extentTestInfo("Rapora ekran görüntüsü eklenirken hata meydana geldi (An error occurred while adding the screenshot to the report.)");
        } finally {
            Driver.closeDriver();
        }
    }
